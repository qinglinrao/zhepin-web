...
class CustomPresenter extends Illuminate\Pagination\Presenter {
	...
	{         
            ...
            return $this->getPageLinkWrapper($url, $page, $rel);
	}
        ...
        public function getUrl($page){
            ...
            if($page > 1){
                if(preg_match("/page-[0-9]+$/", Request::url())){
                    $url = preg_replace("/page-[0-9]+$/", 'page-'.$page, Request::url());
                }else{
                    $url = Request::url().'/page-'.$page;
                }
                $url = $url.Config::get('app.url_suffix','');
            }else{
                $url = preg_replace("/(page-[0-9]+)$/", '', Request::url());
            }

            $url = $parameters ? $url.'?'.http_build_query($parameters, null, '&') : $url;
